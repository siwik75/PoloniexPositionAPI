...
class poloniex:
    ...
    @staticmethod
    def createTimeStamp(datestr, format=DATE_FORMAT):
        timeNOTZ = time.mktime(time.strptime(datestr, format))
        return timeNOTZ + (3600 * poloniex.DATE_TZ)
                ...
                for x in xrange(0, len(after['return'])):
                    if isinstance(after['return'][x], dict):
                        if 'datetime' in after['return'][x] and 'timestamp' not in after['return'][x]:
                            after['return'][x]['timestamp'] = float(poloniex.createTimeStamp(after['return'][x]['datetime']))
    ...
    def returnTradeHistory(self,currencyPair, start=None, end=None):
        ...
        assert (type(start)==float and (type(end)==float or not end)) or (not start and not end), "start and/or end MUST be an UNIX integer representing timestamp."
        if start and not end:
            end = self.LAST_DATE

        if start or end:
            print "Requesting %s - start %s / end %s" % (currencyPair, start, end)
            return self.api_query('returnTradeHistory', {"currencyPair":currencyPair, "start": start, "end": end})
        else:
            return self.api_query('returnTradeHistory', {"currencyPair":currencyPair,  "limit": 10000,
                                                         "start":12900000})
